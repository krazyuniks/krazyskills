...
3. Run the bundled preflight and CLI:
   `<skill-dir>/bin/backhand handoff --focus "<focus>" --backend tmux`
...
5. Add `--profile`, `--tmux-dispatch-path`, `--tmux-harness`, `--tmux-effort`, or `--timeout` only
...
The current working path is Claude Code JSONL source plus the tmux backend. If another source or
